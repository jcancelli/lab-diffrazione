...
myfit("nomefile.txt", background, normalizzazione, lambda, larghezza fenditura, shift lungo x, distanza fenditura - schermo) --> esegue il fit
...
{
	double Arg = TMath::Pi() * par[0] * (x[0] - par[1]) / TMath::Sqrt((x[0] - par[1]) * (x[0] - par[1]) + par[2] * par[2]) / par[3];
	double Diffr = par[5] + par[4] * sin(Arg) * sin(Arg) / Arg / Arg;
	return Diffr;
}

void myfunc(double bkg = 1., double Norm = 500., double lambda = 632.8E-9, double d = 1.E-4, double x0 = 0.057, double L = 1.0)
{
...
void mydata(TString fname = " ")
{
	...
	data->GetXaxis()->CenterTitle(true);
	c->SaveAs("ciaccia.png");
}

void myfit(TString fname = " ", double bkg = 2., double Norm = 500., double lambda = 632.8E-9, double d = 1.E-4, double x0 = 0.057, double L = 1.0)
{
	TGraphErrors *data = new TGraphErrors(fname, "%lg %lg %lg");
